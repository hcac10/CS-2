...
public class NGramMap {
    ...
    public IPriorityQueue.PQElement<String>[] getCountsAfter(NGram ngram) {
        IDictionary<IterableString, Integer> temp = map.get(ngram);
        //IDictionary<IterableString, Integer> other = inner.get();
        if(temp == null)
            return null;
        IPriorityQueue.PQElement<String>[] ans = new IPriorityQueue.PQElement[temp.size()];
        int acc = 0;
        for(IterableString c: temp.keySet()){
            int counts = temp.get(c);
            ans[acc] = new IPriorityQueue.PQElement(c.toString(), counts);
            acc++;
        }
        return ans;
    }
...
//    private Supplier<IDictionary<IterableString, Integer>> inner;
    ...
    public String[] getWordsAfter(NGram ngram, int k) {
        IPriorityQueue.PQElement<String>[] temp = getCountsAfter(ngram);
        TopKSort a = new TopKSort();
        a.sort(temp, k);
        String[] ans = new String[Math.min(map.get(ngram).size(),k)];
        for(int i = 0; i < Math.min(map.get(ngram).size(),k); ++i){
            ans[i] = temp[i].data;
        }
        return ans;
    }
